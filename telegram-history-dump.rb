...
def process_media(dialog, msg)
  ...
  %w(document video photo audio).each do |media_type|
    ...
    $log.info('Downloading %s' % media_type)
    begin
    ...
    filename = case
      when $config['copy_media']
        ...
        # Do ext conversion here?
        destination = File.join(get_media_dir(dialog), filename)
...
require_relative 'dumpers/' + $config['dumper']
$dumper = Dumper.new
...
dialogs.each do |dialog|
  ...
  if backup_target?(dialog)
    backup_list.push(dialog) else skip_list.push(dialog) end
end
